...
import time
from proto.pos_service_pb2 import GetProductPriceRequest, GetProductPriceResponse, UpdateProductPriceRequest, UpdateProductPriceResponse
...
class POSServicer(pos_service_pb2_grpc.POSServicer):
    def __init__(self, deposit: Deposit, node_id: str, role: Role, peers: list, host: str, port: int):
    ...
    def UpdateProductPrice(self, request, context):
        ...
        if self.role != Role.LEADER:
            return UpdateProductPriceResponse(message="Not authorized: Only the leader can update prices.")
        
        # The leader updates the price
        success = self.deposit.change_price(request.product_id, request.new_price)
        if success:
            print(f"Price for product {request.product_id} updated to {request.new_price}.")
            # Notify all peers (followers) of the price change
            self.notify_peers(request.product_id, request.new_price)
            return UpdateProductPriceResponse(success=True, message="Price updated successfully")
        else:
            return UpdateProductPriceResponse(success=False, message="Product not found")
    ...
    def notify_peers(self, product_id, new_price):
        ...
        for peer in self.peers:
            ...
            try:
                print(f"Notifying peer at {host}:{port} to update price for product {product_id} to {new_price}")
                # Notify peers via gRPC to update the price
                channel = grpc.insecure_channel(f'{host}:{port}')
                stub = pos_service_pb2_grpc.POSStub(channel)
                update_request = UpdateProductPriceRequest(product_id=product_id, new_price=new_price)
    ...
    def NotifyPeersToUpdatePrice(self, request, context):
        ...
        print(f"Peer {self.node_id} received price update for product {request.product_id} to {request.new_price}")
        success = self.deposit.change_price(request.product_id, request.new_price)
        if success:
            return UpdateProductPriceResponse(success=True, message="Price updated successfully")
        else:
...
def serve():
    ...
    node_1 = POSServicer(deposit1, "POS1", Role.LEADER, [('localhost', 50052), ('localhost', 50053)], 'localhost', 50051)
    node_2 = POSServicer(deposit2, "POS2", Role.FOLLOWER, [('localhost', 50051)], 'localhost', 50052)
    node_3 = POSServicer(deposit3, "POS3", Role.FOLLOWER, [('localhost', 50051)], 'localhost', 50053)
